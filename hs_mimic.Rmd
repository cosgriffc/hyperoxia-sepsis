...
Lets recode ethnicity since there are too many groups based on the MIMIC
ethnicities. **move this up later**
```{r}
ethnicities <- c("WHITE", "BLACK", "HISPANIC", "ASIAN")

for (i in 1:length(ethnicities)) {
  hs.eda.df$ethnicity[str_detect(hs.eda.df$ethnicity, ethnicities[i])] <- ethnicities[i]
}
hs.eda.df$ethnicity[!(hs.eda.df$ethnicity %in% ethnicities)] <- "OTHER"
hs.eda.df$ethnicity <- relevel(hs.eda.df$ethnicity, ref = "WHITE")
```

Also re-level care unit. **Move this up later too**
```{r}
hs.eda.df$first_careunit <- relevel(hs.eda.df$first_careunit, "MICU")
```
...
```{r}
abstract.model <- glm(hospital_expire ~ hyperoxic_group, family = binomial(link = "logit"),
                      data = hs.eda.df)
summary(abstract.model)
abstract.model <- glm(hospital_expire ~ hyperoxic_group + age + gender + 
...
```{r}
sjPlot::plot_model(abstract.model, terms = c("hyperoxic_group4", "hyperoxic_group3", "hyperoxic_group2"),  order.terms = c(3, 2, 1), axis.labels = c("Hyperoxic Group 2", "Hyperoxic Group 3", "Hyperoxic Group 4"), show.values = TRUE, title = "Hospital Expiration vs. Hyperoxic Group", show.p = FALSE, axis.title = "Adjusted Odds Ratio")
```


```{r}
library(broom)
tidy(abstract.model, conf.int = TRUE, exponentiate = TRUE)
```
