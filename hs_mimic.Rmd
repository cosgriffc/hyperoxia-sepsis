...
cohort.tidy.df <- cohort.df %>% group_by(subject_id, hadm_id, icustay_id, gender, 
  ...
  summarise(age = mean(age), icu_los = mean(icu_length_of_stay), 
            ...
            hospital_expire = any(hospital_expire_flag == 1),
            oneyear_expire = any(oneyear_expire == 1)) %>% ungroup() %>%
  select(subject_id, icustay_id, age, gender, ethnicity, first_careunit, elixhauser, oasis, icu_los, vent_duration, 
         angus, vasopressor_flag, hospital_expire, oneyear_expire, exclusion_icu_expire, exclusion_age, 
         exclusion_los)
