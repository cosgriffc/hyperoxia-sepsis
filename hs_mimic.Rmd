---
title: 'The Role of Hyperoxia in Sepsis Mortality'
...
```{r}
cohort.df$age[cohort.df$age>100] <- 91.4
```
...
cohort.df$ethnicity <- as.factor(cohort.df$ethnicity)
cohort.df$ethnicity <- relevel(cohort.df$ethnicity, ref = "WHITE")
```

Also re-level care unit.
```{r}
cohort.df$first_careunit <- as.factor(cohort.df$first_careunit)
cohort.df$first_careunit <- relevel(cohort.df$first_careunit, "MICU")
```
...
spO2.columns <- c("row_id", "subject_id", "hadm_id", "icustay_id", "item_id", 
              ...
              "stopped")
spO2 <- as.data.frame(read_csv("./data/spO2.csv.gz", col_names = spO2.columns))
```
...
#vent.df <- as.data.frame(fread(paste0("\"C:\\Program Files\\Git\\bin\\sh.exe\" zcat < ", "./data/ventdurations.csv.gz"), showProgress = FALSE)) 
vent.columns <- c("icustay_id", "ventnum", "starttime", "endtime", "duration_hours")
vent.df <- as.data.frame(read_csv("./data/ventdurations.csv.gz", col_names = vent.columns))
vent.df <- vent.df %>%
  filter(icustay_id %in% cohort.tidy.df$icustay_id) %>% 
  select(icustay_id, starttime, endtime)
```
...
hs.noHypoxia.df$hyperoxic_quartile <- with(hs.noHypoxia.df, cut(hyperoxic_duration, 
                                breaks = quantile(hyperoxic_duration, 
                                                  ...
                                                  na.rm=TRUE), 
                                include.lowest = TRUE))
hyperoxia.levels <- levels(hs.noHypoxia.df$hyperoxic_quartile)
...
```{r}
CreateTableOne(vars=c("age","gender", "ethnicity", "first_careunit", "icu_los", "vent_duration",
                      "elixhauser", "oasis", "vasopressor_flag"), strata = "hyperoxic_group", 
               data = hs.eda.df, test = TRUE)
```

Distribution of hyperoxia?
```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = hyperoxic_duration)) +
  xlab("Hyperoxic Duration")
```

By group?

```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = hyperoxic_duration, fill = hospital_expire)) + 
  facet_wrap(~hyperoxic_group) + xlab("Hyperoxic Duration") + 
  guides(fill = guide_legend(title="Hospital Expiration")) + ggtitle("Hyperoxia by Group")
```

Distribution of vent durations?

```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = vent_duration))
```
Distribution of LOS?

```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = icu_los))
```

Association between these?
```{r}
ggplot(hs.eda.df) + 
  geom_point(aes(x = vent_duration, y = hyperoxic_duration), alpha = 0.5) +
  xlab("Ventilation Duration") + ylab("Hyperoxic Duration")
```

```{r}
ggplot(hs.eda.df) + 
  geom_point(aes(x = icu_los, y = hyperoxic_duration), alpha = 0.5) +
  xlab("ICU Length of Stay") + ylab("Hyperoxic Duration")
```

Is there a clear association with death here?
```{r}
ggplot(hs.eda.df) + geom_point(aes(x = vent_duration, y = hyperoxic_duration, size = icu_los, color = hospital_expire), alpha = 0.5)
```

How does hyperoxic duration correlate with OASIS and Elixhauser? Any pattern for death?
```{r}
ggplot(hs.eda.df) + geom_point(aes(x = oasis, y = hyperoxic_duration, color = hospital_expire), alpha = 0.5)
```

Age association?

```{r}
ggplot(hs.eda.df) + geom_point(aes(x = age, y = hyperoxic_duration))
```

```{r}
ggplot(hs.eda.df) + geom_point(aes(x = age, y = hyperoxic_duration, color = hospital_expire))
```

```{r}
ggplot(hs.eda.df) + geom_point(aes(x = age, y = hyperoxic_duration, color = first_careunit))
```

```{r}
ggplot(hs.eda.df) + geom_boxplot(aes(x = as.factor(elixhauser), y = hyperoxic_duration, color = hospital_expire), alpha = 0.5)
```

How do the above distributions change by our factor variables?
```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = hyperoxic_duration)) + facet_wrap(~gender)
```

```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = hyperoxic_duration)) + facet_wrap(~ethnicity)
```

```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = hyperoxic_duration)) + facet_wrap(~first_careunit)
```

```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = hyperoxic_duration)) + facet_wrap(~vasopressor_flag)
```

Since we categorized, I also want to see some of these features across the hyperoxic groups
```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = icu_los)) + facet_wrap(~hyperoxic_group)
```

Outcome, outcome by group plots

```{r}
ggplot(hs.eda.df) + geom_histogram(aes(x = hyperoxic_duration)) + facet_wrap(~hospital_expire)
```


```{r}
plot_prop_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), "hyperoxic_group", "hospital_expire")
```

```{r}
plot_prop_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), 
                   "hyperoxic_group", "hospital_expire", factor.var2 = "ethnicity") +
  xlab("Hyperoxic Group") + ylab("Proportion Died") + 
  guides(color = guide_legend(title="Ethnicity"))
```

```{r}
plot_prop_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), "hyperoxic_group", "hospital_expire", factor.var2 = "ethnicity")
```

```{r}
plot_prop_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), 
                   "hyperoxic_group", "hospital_expire", factor.var2 = "first_careunit")
```

```{r}
plot_prop_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), 
                   "hyperoxic_group", "hospital_expire", factor.var2 = "vasopressor_flag")
```

```{r}
plot_OR_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), 
                 "hyperoxic_group", "hospital_expire") + 
  ggtitle("Hospital Expiration vs. Hyperoxic Group (Crude)") + xlab("Hyperoxic Group")
```

```{r}
plot_OR_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), "hyperoxic_group", "hospital_expire", factor.var2 = "gender")
```

```{r}
plot_OR_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), "hyperoxic_group", "hospital_expire", factor.var2 = "ethnicity")
```

```{r}
plot_OR_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), "hyperoxic_group", "hospital_expire", factor.var2 = "first_careunit")
```

```{r}
plot_OR_by_level(hs.eda.df %>% mutate(hospital_expire = as.numeric(.$hospital_expire == TRUE)), "hyperoxic_group", "hospital_expire", factor.var2 = "vasopressor_flag")
```
...
```{r}
crude_model <- glm(hospital_expire ~ hyperoxic_group, family = binomial(link = "logit"),
                      data = hs.eda.df)
summary(crude_model)
...
```{r}
adj_model <- glm(hospital_expire ~ hyperoxic_group + age + gender + 
                        ethnicity + first_careunit + icu_los + vent_duration + 
                        oasis + elixhauser + vasopressor_flag, family = binomial(link = "logit"),
                      data = hs.eda.df)
summary(adj_model)
tidy(adj_model, exponentiate = TRUE, conf.int = TRUE)
```

*Hoslem Goodness of Fit*
```{r}
hoslem_gof(adj_model)$p.value
```

### Visualizing the Results
```{r, warning = FALSE, message= FALSE}
sjPlot::plot_model(crude_model, terms = c("hyperoxic_group4", "hyperoxic_group3", "hyperoxic_group2"),  order.terms = c(3, 2, 1), axis.labels = c("Hyperoxic Group 2", "Hyperoxic Group 3", "Hyperoxic Group 4"), show.values = TRUE, title = "Hospital Expiration vs. Hyperoxic Group (Crude)", show.p = FALSE, axis.title = "Unadjusted Odds Ratio", type = "est")
sjPlot::plot_model(adj_model, terms = c("hyperoxic_group4", "hyperoxic_group3", "hyperoxic_group2"),  order.terms = c(3, 2, 1), axis.labels = c("Hyperoxic Group 2", "Hyperoxic Group 3", "Hyperoxic Group 4"), show.values = TRUE, title = "Hospital Expiration vs. Hyperoxic Group (Adjusted)", show.p = FALSE, axis.title = "Adjusted Odds Ratio", type = "est")
```
