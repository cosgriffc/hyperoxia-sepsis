---
title: 'Hyperoxia & Sepsis: Exploring No hypoxia exlcusion'
...
## Load up the base cohort
```{r}
cohort.df <- dbGetQuery(mimic, 
                        "SELECT * FROM mimiciii.hyperoxia_sepsis_sensitivity")
```
...
## Collapase flags to subject level
```{r}
cohort.collapse <- cohort.df %>% 
  group_by(subject_id) %>% 
  summarise()
```
